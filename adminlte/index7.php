      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item has-treeview menu-open">
            ...
            <ul class="nav nav-treeview">
              <li class="nav-item">
                <a href="index7.php" class="nav-link active">
                  <i class="far fa-circle nav-icon"></i>
                  <p>Vista citas</p>
                </a>
              </li>
...
<div id="main">
		<h3>Citas Pos/hospitalarias</h3>
          <table class="table table-bordered table-striped">
            ...
            <?php
								$result=mysqli_query($server,"SELECT citaph.*, patients.* FROM citaph, patients WHERE patients.pat_id=citaph.pat_id_citaph ORDER BY id_citaph DESC");
								while($row=mysqli_fetch_assoc($result))
								{
									echo"<tr class=odd>
									  <td>$row[id_citaph]</td>
									  <td>$row[pat_id_citaph]</td>
									  <td>$row[name]</td>
									  <td>$row[lastname]</td>
                    <td>$row[cedula]</td>
                    <td>$row[fechaCita_citaph]</td>
                    <td>$row[fechaRegistro_citaph]</td>                   
                  </tr>";
								}
						?>                       
          </table>
