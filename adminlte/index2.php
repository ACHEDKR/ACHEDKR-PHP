...
<!-- Zona para editar -->
      ...
      <!-- h2 stays for breadcrumbs -->
      <h2>Vista de Todos los Pacientes</h2>
              ...
              <?php
								$result=mysqli_query($server,"SELECT patients.*, pat_to_bed.pat_id, pat_to_bed.bed_id FROM patients, pat_to_bed WHERE patients.pat_id=pat_to_bed.pat_id ORDER BY patients.pat_id DESC");
								while($row=mysqli_fetch_row($result))
								{
									$status="";
									if($row[13]=="none"){ $status="Sin Asignar"; }
									elseif($row[13]>0){ $status="Ingresado <font color=#c66653>{Cama $row[13]}</font>"; } else{ $status="<font color=#33CC99>Dado de Alta</font>"; }
